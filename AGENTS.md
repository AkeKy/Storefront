...
- The final customer-facing identity is **Gadget Arena**. The repository is named **Storefront**. Several technical names still contain `byteforge`; do not rename those keys, package fields, or copy incidentally. A deliberate migration must preserve browser data and update tests.
...
- The Go backend belongs to the separate `project_intern1` Git repository. Do not assume it is present in this checkout or change it unless the user scopes that repository.
