...
export default function CheckoutContent() {
  ...
  if (result) {
    return <div className="flex min-h-screen items-center justify-center px-6 pt-24"><div className="max-w-md text-center"><Icon name="CheckIcon" size={40} className="mx-auto mb-5 text-primary" /><h1 className="text-display-md">{result.mode === 'demo' ? 'PREVIEW COMPLETE' : 'ORDER CONFIRMED'}</h1><p className="mt-4 text-muted-foreground">{result.mode === 'demo' ? 'Demo preview complete. No payment or backend order was submitted, and your cart is unchanged.' : 'Your order request was submitted successfully. Your cart has been cleared.'}</p><Link href="/products" className="btn-primary mt-8">Continue shopping</Link></div></div>;
  }

  return <div className="mx-auto max-w-screen-xl px-6 pb-16 pt-28">
    <Link href="/products" className="text-sm font-semibold text-primary hover:underline">← Continue shopping</Link>
    <h1 className="mt-4 text-display-md">CHECKOUT</h1>
    <p className="mt-3 max-w-3xl text-sm text-muted-foreground">Demo-first checkout: completing this form creates a preview only—no payment or order is sent. A live API order is sent only when a developer has already configured an authenticated session in this browser.</p>
    <div className="mt-8 grid gap-8 lg:grid-cols-5">
      <section className="surface-card p-6 lg:col-span-3">
        <h2 className="text-xl font-bold">Delivery details</h2>
        {hasValidationError && <p role="alert" className="mt-4 rounded-lg border border-destructive p-3 text-sm text-destructive">Enter your name and delivery address, plus contact details, before reviewing your order.</p>}
        {reviewing ? <div className="mt-6">
          <h3 className="font-bold">Review your order</h3>
          <p className="mt-2 text-sm text-muted-foreground">{form.firstName} {form.lastName}, {form.address}, {form.city}, {form.province} {form.postalCode}</p>
          <p className="mt-3 text-sm text-muted-foreground">This normally completes a demo preview. It only submits a live order when a developer-configured authenticated API session is already available in this browser.</p>
          {submissionError && <div role="alert" className="mt-4 rounded-lg border border-destructive p-3 text-sm text-destructive">{submissionError}<button className="ml-3 font-bold underline" onClick={submitOrder} disabled={isSubmitting}>Retry</button></div>}
          <div className="mt-6 flex gap-3"><button className="btn-outline" onClick={() => setReviewing(false)} disabled={isSubmitting}>Edit details</button><button className="btn-primary" onClick={submitOrder} disabled={items.length === 0 || isSubmitting}>{isSubmitting ? 'Submitting order...' : `Submit order · ${currency.format(subtotalTHB)}`}</button></div>
        </div> : <>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">{requiredFields.map((key) => {
          ...
          <button className="btn-primary mt-6 w-full justify-center" onClick={reviewOrder}>Review order</button>
        </>}
      </section>
      <aside className="surface-card h-fit p-6 lg:col-span-2">
        <h2 className="text-xl font-bold">Order summary <span className="text-sm text-muted-foreground">({itemCount})</span></h2>
        {items.length === 0 ? <p className="mt-4 text-muted-foreground">Your cart is empty.</p> : <div className="mt-5 space-y-4">{items.map((item) => <div key={item.product.id} className="flex gap-3"><div className="relative h-16 w-16 overflow-hidden rounded bg-muted"><AppImage src={item.product.image} alt={item.product.imageAlt} fill sizes="64px" className="object-cover" /></div><div className="min-w-0 flex-1"><p className="font-semibold">{item.product.name}</p><p className="text-sm text-muted-foreground">{currency.format(item.product.priceTHB)}</p><div className="mt-2 flex items-center gap-2"><button aria-label={`Decrease ${item.product.name} quantity`} onClick={() => updateQuantity(item.product.id, item.quantity - 1)} disabled={isSubmitting}>−</button><span>{item.quantity}</span><button aria-label={`Increase ${item.product.name} quantity`} onClick={() => updateQuantity(item.product.id, item.quantity + 1)} disabled={isSubmitting}>+</button><button className="ml-auto text-sm text-destructive" onClick={() => removeItem(item.product.id)} disabled={isSubmitting}>Remove</button></div></div></div>)}</div>}
        <div className="mt-6 space-y-2 border-t pt-4 text-sm"><p className="flex justify-between"><span>Subtotal</span><span>{currency.format(subtotalTHB)}</span></p><p className="flex justify-between text-lg font-bold"><span>Total</span><span>{currency.format(subtotalTHB)}</span></p></div>
        <p className="mt-4 text-xs text-muted-foreground">Delivery quote unavailable in demo. No payment is collected in this checkout.</p>
      </aside>
    </div>
  </div>;
}
