...
export default function CheckoutContent() {
  ...
  if (result) return <div className="flex min-h-screen items-center justify-center px-6 pt-24"><div className="max-w-md text-center"><Icon name="CheckIcon" size={40} className="mx-auto mb-5 text-primary" /><h1 className="text-display-md">ORDER CONFIRMED</h1><p className="mt-4 text-muted-foreground">{result.mode === 'demo' ? 'Demo preview complete. No payment or backend order was submitted.' : 'Your order request was submitted successfully.'}</p><Link href="/products" className="btn-primary mt-8">Continue shopping</Link></div></div>;
